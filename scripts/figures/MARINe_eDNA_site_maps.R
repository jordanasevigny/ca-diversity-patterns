...
# load in data
MARINe_df <- data.frame(read.csv("./data/preliminary_processed_data/MARINe_survey_taxa_dmy.csv", header=TRUE)) %>%
  filter(year %in% 2001) %>%
  select(c("marine_site_name", "latitude", "longitude")) %>%
  distinct() %>%
  rename(Sites = marine_site_name, Latitude = latitude, Longitude = longitude) %>%
...
ggplot(data = world) +
  ...
  coord_sf(xlim = c(-110, -130), ylim = c(25, 45), expand = FALSE) +
  facet_wrap(~ Source) 
ggsave("preliminary_figures/MARINe_2001_eDNA_site_map.png")
...
ggplot(data = world) +
  ...
  coord_sf(xlim = c(-110, -130), ylim = c(25, 45), expand = FALSE) +
  facet_wrap(~ Source) 
ggsave("preliminary_figures/MARINe_2002_eDNA_site_map.png")
...
ggplot(data = world) +
  ...
  coord_sf(xlim = c(-110, -130), ylim = c(25, 45), expand = FALSE) +
  facet_wrap(~ Source) 
ggsave("preliminary_figures/MARINe_2003_eDNA_site_map.png")
...
ggplot(data = world) +
  ...
  coord_sf(xlim = c(-110, -130), ylim = c(25, 45), expand = FALSE)
ggsave("preliminary_figures/MARINe_count_2001-2003_eDNA_site_map.png")
...
for(yr in survey_years) {
  ...
  print(p)
  ggsave(filename = paste0("preliminary_figures/MARINe_Sites_map_", yr, ".png"), 
         plot = p)
}
