...
# BL ORIGIN POLYGONS
points_ex_data = pd.read_csv('/home/jsevigny/git/ca-diversity-patterns/data/coast_polygons/wc15n_300km2_settling_polygons.txt', delimiter=',')
...
marine = pd.read_csv('/home/jsevigny/git/ca-diversity-patterns/data/processed_data/biodiversity/marine_species_20241025_dates_20241028_merged.csv')
...
zf = zipfile.ZipFile('/home/jsevigny/git/ca-diversity-patterns/data/marine_data/gbif_marine_biodiversity_20250409.zip') 
...
# Load the CSV file into a DataFrame
PCE = pd.read_csv('/home/jsevigny/git/ca-diversity-patterns/data/edna_jv_data/summer_2024/Summer_Exped_And_Central_Coast_All_Metadata_2024.csv')
...
# Load the CSV file into a DataFrame
peco = pd.read_csv('/home/jsevigny/git/ca-diversity-patterns/data/peco_data/peco_sites_manual.csv')
...
legend_elements = [
    ...
    Line2D([0], [0], color='black', lw=1, label='Release/Settlement Cells')
]
...
plt.xlabel("Longitude")
plt.ylabel("Latitude")
plt.title("Release/Settlement Cells & Sample Sites")

plt.savefig("/home/jsevigny/git/ca-diversity-patterns/figures/biodiversity/map_marine_PCE_PECO_polys.pdf", bbox_inches='tight')
plt.show()
...
# Add sample sites
# Filter the MARINe to only rows where latitude < 42 (~PCE bounds)
filtered = unique_coords_marine_ex[unique_coords_marine_ex['decimalLatitude'] < 38]
ax.scatter(filtered['decimalLongitude'], filtered['decimalLatitude'], c='#00BFC4', marker='*')
#ax.scatter(unique_coords_peco['Longitude'], unique_coords_peco['Latitude'], c='#C77CFF', marker='*')
ax.scatter(unique_coords_PCE['Longitude'], unique_coords_PCE['Latitude'], c='#F8766D', marker='*')
...
for idx, row in polygon_df.iterrows():
    ...
    # Limit settlement polygons to max lat of PCE
    if polygon.bounds[3] < 38.0:  # bounds = (minx, miny, maxx, maxy)
        x, y = polygon.exterior.xy
        ax.plot(x, y, alpha=0.9, color='black', linewidth=0.5)
...
# Custom legend handles
legend_elements = [
    Line2D([0], [0], marker='*', color='w', label='MARINe-CA Sites', markerfacecolor='#00BFC4', markersize=10),
    Line2D([0], [0], marker='*', color='w', label='PCE Sites', markerfacecolor='#F8766D', markersize=10),
  #  Line2D([0], [0], marker='*', color='w', label='PECO Sites', markerfacecolor='#C77CFF', markersize=10),
    Line2D([0], [0], color='black', lw=1, label='Release/Settlement Cells')
]
...
plt.xlabel("Longitude")
plt.ylabel("Latitude")
plt.title("Release/Settlement Cells & MARINe-CA and PCE Sample Sites")
ax.set_extent([-126, -111, 25, 40], crs=ccrs.PlateCarree())

#plt.savefig("/home/jsevigny/git/ca-diversity-patterns/figures/biodiversity/map_marine_PCE_polys.pdf", bbox_inches='tight')
plt.show()
...
# Custom legend handles
legend_elements = [
    Line2D([0], [0], marker='*', color='w', label='MARINe/PISCO Sites', markerfacecolor='#00BFC4', markersize=10),
  #  Line2D([0], [0], marker='*', color='w', label='PCE Sites', markerfacecolor='#F8766D', markersize=10),
    Line2D([0], [0], marker='*', color='w', label='PECO Sites', markerfacecolor='#C77CFF', markersize=10),
  #  Line2D([0], [0], color='black', lw=1, label='Release/Settlement Cells')
]
...
plt.xlabel("Longitude")
plt.ylabel("Latitude")
plt.title("Release/Settlement Cells & MARINe/PISCO and PECO Sample Sites")
#ax.set_extent([-126, -111, 25, 40], crs=ccrs.PlateCarree())

plt.savefig("/home/jsevigny/git/ca-diversity-patterns/figures/biodiversity/map_marine_peco.pdf", bbox_inches='tight')
plt.show()
